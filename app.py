...
def register():
    if request.method == 'POST':
        ...
        if existing_user:
            flash('Это имя пользователя уже используется', 'error')
        else:
...
def pievienosana():
    if request.method == 'POST':
        ...
        return redirect(url_for('pievienosana'))  # Перенаправляем на страницу с финансовым отчетом после добавления записи
    else:
